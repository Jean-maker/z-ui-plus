...
export const rule = {
   ...
   *
   */
  checkWorkshopLine(rule: any, value: string[], callback: any) {
    if (value?.length != 2) {
      callback(new Error("请选择车间产线"));
    } else {
      callback();
    }
  },
   ...
   *
   */
  checkWorkshopLineProcedure(rule: any, value: string[], callback: any) {
    if (value?.length != 3) {
      callback(new Error("请选择车间产线工序"));
    } else {
      callback();
    }
  },
  ...
  /* 金额数字 */
  decimalNumber(rule, value, callback) {
    validateFn("decimalNumber", rule, value, callback, "包含非数字字符");
  },
  /* 字母 */
